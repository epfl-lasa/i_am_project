...
from py_dual_passive_inertial_control import PassiveControl
...
DES_ORI_ROBOT_LEFT = np.array([0.3827, -0.9239, 0.0, 0.0]) # w x y z
DES_POS_ROBOT_RIGHT = np.array([0.35,  -0.20, 0.80])
DES_ORI_ROBOT_RIGHT = np.array([0.3827,  0.9239, 0.0, 0.0]) # w x y z
...
# -- passive ds hitting
hit_direction = [0, 1, 0]
iiwa_return_position = [0.33, -0.3, 0.32]

des_pos_hitting = np.array([0.33, -0.3, 0.32])
des_quat_hitting = np.array([0.707, -0.707 , 0.0, 0.0]) # w x y z
...
def init_passive_controllers(robot_left, robot_right):
    controller_left = PassiveControl(open('urdf/iiwa7.urdf').read(), END_EFFECTOR_LEFT)
    ...
    controller_right = PassiveControl(open('urdf/iiwa14.urdf').read(), END_EFFECTOR_RIGHT)
...
def init_hitting_ds(ee_pos, box):
    box_position = box.position - np.array([0, -0.31, 0.75]) # box length and iiwa pos
    hitting_controller = hitting_DS(ee_pos, box_position)
    hitting_controller.set_des_direction(hit_direction)
    return hitting_controller

def generate_hitting_ee_vel(is_hit, hitting_generation, iiwa_task_inertia_pos, box_pos_before, box_pos_after):
    if not is_hit:
    ...
    # if not is_hit and np.linalg.norm(box_pos_before - box_pos_after) > 0.005:
    print("EE POS ", hitting_generation.get_current_position())
    print("DS ATTRACTOR ", hitting_generation.get_DS_attractor())
...
if __name__ == '__main__':
    ...
    hitting_generation = init_hitting_ds(controller_left.getEEpos(), box)
    ...
    init_pose_hit = False
    hitting = False
    cycle_init_pos_hit = 0
    is_hit = False
    cycle_is_hit = 0

    while True:
        # Update controllers
        box_pos_before = box.position
        get_agx_sensors(robot_left, robot_right, box)
        controller_left.updateRobot(robot_left.joint_position,
                                    robot_left.joint_velocity, robot_left.joint_effort)
        controller_right.updateRobot(robot_right.joint_position,
        ...
        hitting_generation.set_current_position(controller_left.getEEpos())

        if not robots_init_pos and np.linalg.norm(box.position[:2] - np.array([0.33, 0.0])) > 0.05:
            # HIT NEEDED
            controller_left.set_desired_pose(des_pos_hitting, des_quat_hitting)
            hitting = True

        if not robots_init_pos and \
        (
            (not hitting and np.linalg.norm(DES_POS_ROBOT_LEFT - controller_left.getEEpos()) < 0.001) or \
            (hitting and np.linalg.norm(des_pos_hitting - controller_left.getEEpos()) < 0.0035)
        ) and \
        np.linalg.norm(DES_POS_ROBOT_RIGHT - controller_right.getEEpos()) < 0.001 and \
        (
            hitting or (np.linalg.norm(DES_ORI_ROBOT_LEFT - controller_left.getEEquat()) < 0.01 or \
            np.linalg.norm(DES_ORI_ROBOT_LEFT + controller_left.getEEquat()) < 0.01)
        ) and \
        ...
        if robots_init_pos:
            ...
            if hitting:
                # print("DIFF ", np.linalg.norm(des_pos_hitting - controller_left.getEEpos()))
                if np.linalg.norm(des_pos_hitting - controller_left.getEEpos()) < 0.0045:
                    cycle_init_pos_hit = cycle_init_pos_hit + 1
                    if cycle_init_pos_hit >= max_cycle_init_pos + 10:
                        init_pose_hit = True
            
                if init_pose_hit:
                    ref_velocity_ee, is_hit = generate_hitting_ee_vel(
                        is_hit, hitting_generation, controller_left.getTaskInertiaPos(), box_pos_before, box.position)
                    controller_left.set_desired_velocity(ref_velocity_ee)
                    print("REF VEL ", ref_velocity_ee)
            
                if np.linalg.norm(box.position[:2] - np.array([0.33, 0.0])) < 0.05:
                    is_hit = True

                if is_hit:  
                    cycle_is_hit = cycle_is_hit + 1
                    if cycle_is_hit >= 100:
                        hitting = False
                        is_hit = False
                        controller_left.set_desired_pose(DES_POS_ROBOT_LEFT, DES_ORI_ROBOT_LEFT)
