...
geometry_msgs::Pose block(Eigen::Vector3d object_pos, Eigen::Vector3d predict_pos, Eigen::Vector4d block_quat, Eigen::Vector3d iiwa_base_pos, Eigen::Vector3d iiwa_flip, const int iiwa_no){
  ...
  pose.position.x = (object_pos[0] + (iiwa_base_pos[1]+0.1*flip_vec[1]-object_pos[1])*(predict_pos[0]-object_pos[0])/(predict_pos[1]-object_pos[1]) - iiwa_base_pos[0])*flip_vec[0]; //calculate x on the predicted trajectory at point of interception with line y = just next to iiwa_base
