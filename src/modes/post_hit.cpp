...
geometry_msgs::Pose postHit(const Eigen::Vector3d object_pos_init, const Eigen::Vector4d rest_quat, const Eigen::Vector3d iiwa_base_pos, Eigen::Vector3d iiwa_flip, const int iiwa_no){
  ...
  pos_quat.position.x = (object_pos_init[0] - iiwa_base_pos[0])*flip_vec[0];
  pos_quat.position.y = (object_pos_init[1] - iiwa_base_pos[1])*flip_vec[1];
  pos_quat.position.z = (object_pos_init[2] - iiwa_base_pos[2])*flip_vec[2];
