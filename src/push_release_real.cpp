...
void PushRelease::iiwaBasePositionCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
...
void PushRelease::iiwaPositionCallback(const geometry_msgs::Pose::ConstPtr& msg) {
...
void PushRelease::iiwaVelCallback(const geometry_msgs::Twist::ConstPtr& msg) {
  iiwa_vel_from_source_ << msg->linear.x, msg->linear.y, msg->linear.z;
}

void PushRelease::objectPositionCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
...
void PushRelease::objectPositionWorldFrame() {
...
void PushRelease::iiwaInertiaCallback(const geometry_msgs::Inertia& inertia_msg) {
...
void PushRelease::setGains(Eigen::Matrix3f& gain) { generate_hitting_->set_gain(gain); }

void PushRelease::updateCurrentEEPosition(Eigen::Vector3f& new_position) {
  generate_hitting_->set_current_position(new_position);
}

void PushRelease::updateCurrentObjectPosition(Eigen::Vector3f& new_position) {
  generate_hitting_->set_DS_attractor(new_position);
}

void PushRelease::publishPosQuat(const Eigen::Vector3f& DS_pos, const Eigen::Vector4f& DS_quat) {
...
void PushRelease::publishVelQuat(Eigen::Vector3f& DS_vel, Eigen::Vector4f& DS_quat) {
...
void PushRelease::publishFlux(Eigen::Matrix3f& iiwa_task_inertia_pos, Eigen::Vector3f& iiwa_vel_from_source) {
...
int main(int argc, char** argv) {
  ...
  std::unique_ptr<PushRelease> generate_motion = std::make_unique<PushRelease>(nh, frequency);
