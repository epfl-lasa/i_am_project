...
std::tuple<Eigen::MatrixXd, Eigen::MatrixXd> kalmanFilter(Eigen::Matrix<double,9,9> P_prior, Eigen::Matrix<double,9,1> s_prior, Eigen::Vector3d object_pos, Eigen::Vector3d object_vel){
    ...
    Eigen::MatrixXd C(6,9);
    Eigen::Matrix3d I3 = Eigen::Matrix3d::Identity();
    Eigen::Matrix3d Z3 = Eigen::Matrix3d::Zero();
    syst << I3, I3*dt, Z3, Z3, I3, I3*dt/0.3, Z3, Z3, I3;
    C << I3, Z3, Z3, Z3, I3, Z3;

    Eigen::Matrix<double, 9,9> W;
    W << 1*I3, Z3, Z3, Z3, 0.2*I3, Z3, Z3, Z3, 1*I3;
    Eigen::Matrix<double, 6,6> V = 1*Eigen::Matrix<double, 6,6>::Identity();



    //Covariance update
    Eigen::Matrix<double, 9,9> P_pred;
    Eigen::Matrix<double, 9,9> P_post;
    ...
    //State update
    Eigen::Matrix<double, 9,1> s_pred;
    Eigen::Matrix<double, 9,1> s_post;
    
    Eigen::Matrix<double, 6,1> output;
    output << object_pos, object_vel;
    ...
    s_post = s_pred + P_post*C.transpose()*V.inverse()*(output - C*s_pred);             //correct current state given information of current measurement
...
std::tuple<Eigen::MatrixXd, Eigen::MatrixXd, Eigen::Vector3d, double> predictPos(Eigen::Matrix<double,9,9> P, Eigen::Matrix<double,9,1> state, Eigen::Vector3d object_pos, Eigen::Vector3d object_vel, double mass){
  std::tie(P,state) = kalmanFilter(P,state,object_pos,object_vel); //estimate current covar and state given priors and measurement

  Eigen::Vector3d vel_est;
  Eigen::Vector3d f_est;
  double ETA;

  vel_est << state[3], state[4], state[5];
  f_est << state[6], state[7], state[8];
